...
export default function TokenOperationDetail() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (result.calls?.length > 0) {
        ...
        const totalPrompt = calls.reduce((sum, c) => sum + (c.prompt_tokens || 0), 0);
        const totalCompletion = calls.reduce((sum, c) => sum + (c.completion_tokens || 0), 0);
  ...
  const handleRowClick = (row) => {
    navigate(`/stories/calls/${row.call_id}?from=token_imbalance`);
  };
