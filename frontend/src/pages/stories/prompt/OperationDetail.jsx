...
export default function PromptOperationDetail() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (result.calls?.length > 0) {
        ...
        const totalPromptTokens = calls.reduce((sum, c) => sum + (c.prompt_tokens || 0), 0);
        const avgPromptTokens = calls.length ? Math.round(totalPromptTokens / calls.length) : 0;
  ...
  const handleRowClick = (row) => {
    navigate(`/stories/calls/${row.call_id}?from=system_prompt`);
  };
