      ...
      <table className="w-full text-base table-fixed">
        <thead className={`${BASE_THEME.container.secondary} border-b ${BASE_THEME.border.default}`}>
          <tr className={`text-left ${BASE_THEME.text.secondary}`}>
            <th className="py-4 px-3 md:px-6 font-medium" style={{ width: '40%' }}>Cause</th>
            <th className="py-4 px-3 md:px-6 font-medium" style={{ width: '20%' }}>Impact</th>
            <th className="py-4 px-3 md:px-6 font-medium" style={{ width: '18%' }}>Severity</th>
            <th className="py-4 px-3 md:px-6 font-medium text-right" style={{ width: '22%' }}>Action</th>
          </tr>
        </thead>
        <tbody>
          {sortedCauses.map((cause, idx) => (
            ...
            >
              ...
              <td className="py-5 px-3 md:px-6">
                ...
                {cause.description && (
                  <div className={`text-sm md:text-base ${BASE_THEME.text.secondary} leading-relaxed`}>{cause.description}</div>
                )}
              </td>

              {/* Impact */}
              <td className="py-5 px-3 md:px-6">
                <span className={`font-mono text-sm md:text-base font-semibold ${getSeverityTextColor(cause.severity)} break-words`}>
                  {cause.impact}
                </span>
              </td>
              ...
              <td className="py-5 px-3 md:px-6 text-right">
                {onViewFix && cause.hasFix !== false && (
                  <button
                    ...
                    className={`text-sm px-3 md:px-4 py-2 bg-transparent border-2 ${accentTheme.border} hover:${accentTheme.bg} ${accentTheme.text} hover:text-white rounded transition-colors font-semibold whitespace-nowrap`}
                  >
                    View Fix →
                  </button>
                )}
